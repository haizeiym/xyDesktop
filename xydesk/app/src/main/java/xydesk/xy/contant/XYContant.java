package xydesk.xy.contant;

/**
 * Created by haizeiym
 * on 2016/7/28
 */
public class XYContant {
    private static final int BASE = 0x00000;
    //全局初始化常量
    public static final String F = "FUCK";
    // 删除app
    public static final int DELETER_APP = BASE + 100;

    //添加app
    public static final int ADD_APP = BASE + 101;

    //刷新底部APP
    public static final int REFRESH_BOTTOM_APP = BASE + 103;
    //刷新Fragment
    public static final int REFRESH_FRAGMENT = BASE + 104;

    /**
     * 点击Item
     */
    //打开应用
    public static final String OPEN_APP = "打开应用";
    //应用信息
    public static final String APP_INFO = "应用信息";
    //屏幕图标
    public static final String DESK_APP = "屏幕图标";
    //托盘图标
    public static final String BOTTOM_APP = "托盘图标";
    //添加到哪一个屏幕
    public static final String ADD_DESK_ONE = "添加第一屏";
    public static final String ADD_DESK_TWO = "添加第二屏";
    public static final String ADD_DESK_THREE = "添加第三屏";
    public static final String ADD_DESK_FOUR = "添加第四屏";
    //底部位置
    public static final String BOTTOM_ONE = "位置一";
    public static final String BOTTOM_TWO = "位置二";
    public static final String BOTTOM_THREE = "位置三";
    public static final String BOTTOM_FOUR = "位置四";
    public static final String BOTTOM_FIVE = "位置五";
    //应用卸载
    public static final String DELE_APP = "卸载应用";
    //设置语音名称
    public static final String XFNAME = "添加语音名称";
    /**
     * whatFragment
     */
    //oneFragment
    public static final String ONE_FRAGMENT = "one_parents";
    //twoFragment
    public static final String TWO_FRAGMENT = "two_parents";
    //threeFragment
    public static final String THREE_FRAGMENT = "three_parents";
    //fourFragment
    public static final String FOUR_FRAGMENT = "four_parents";

    /**
     * 长按item
     */
    //在桌面删除
    public static final String DELE_APP_IN_FRAGMENT = "在桌面删除";
    /**
     * 语音设置
     */
    //添加语音名称
    public static final String NAME_SET = "name_set";
    //是否为语音设置
    public static final String IS_VOICE = "isVoice";
    /**
     * 点击菜单
     */
    //全部应用
    public static final String ALL_APP_IN_MENU = "全部应用";
    //语音助手设置
    public static final String APP_SET_IN_MENU = "语音助手设置";
    //联系人别名设置
    public static final String CONTACT_NAME_SET = "联系人别名设置";
}
